...
# -*- coding: utf-8 -*-
"""
Created on Wed Feb  8 22:14:39 2023
...
import imageio, random, time, copy
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
                      ...
                      eval_ssim=False, eval_lpips_alex=False, eval_lpips_vgg=False, cfg=None):
...
def create_new_model_for_acrf(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path, strict=False, device=None):
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None):
    ...
    last_ckpt_path = os.path.join(cfg.basedir, 'ori', cfg.expname, f'{stage}_last.tar')
    if args.no_reload:
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step%args.i_weights==0:
            path = os.path.join(cfg.basedir, 'ori', cfg.expname, f'{stage}_{global_step:06d}.tar')
            torch.save({
...
def voxel2points(voxel):
    ...
    xyz = torch.where(voxel==True)
    xyz_t = []
    for xyz_i in xyz:
        xyz_t.append(xyz_i[:,None])
    
    xyz = torch.cat(xyz_t,-1)        
    return xyz      
...
def load_acdvgo(path, device='cuda'):
    ...
    # loading label_
    labels_ = load_f('labels.npz')  
    num_cluster = labels_.max()+1


    ## loading CT 
    C_rec = torch.zeros((labels_.shape[0],12))
    for i in range(int(num_cluster)):
        CT_binname = path+'/CT_%d.bin'%i
        with open(CT_binname, 'rb') as f:
            strings = f.read()     
        
        head_binname = path+'/head_%d.bin'%i
        with open(head_binname, 'rb') as f:
            min_v, max_v = np.frombuffer(f.read(4*2), dtype=np.int32)
            shape = np.frombuffer(f.read(4*2), dtype=np.int32)
            Qstep = np.frombuffer(f.read(4), dtype=np.int32)/10
            
        ## loading entropy model
        model_path = path+'/eb_%d.pth'%i
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        ...
        ## decompress    
        with torch.no_grad():
            CT_q_dec = entropy_bottleneck.decompress(strings, min_v, max_v, shape, device=device)
    
    
        
        
        ## inv_RAHT
        pos = voxel2points(non_prune_mask.reshape(*world_size))
        ...
        CT_q_dec = CT_q_dec*Qstep[0]
        C_rec_i = inv_haar3D(pos.cpu().numpy()[labels_==i], CT_q_dec, depth) 
        C_rec[labels_==i] = C_rec_i


    ## loading the non-prune density
    ...
    full_grid[non_prune_mask,:] = C_rec
...
def init_importance_direct(model, render_poses, HW, Ks, ndc, render_kwargs, savedir=None, render_factor=0, cfg=None):
    '''view-independent pruning 
    '''
    imp_path = os.path.join(savedir, 'importance_direct.pth')
    ...
    factor = 2
    samp_pts = torch.stack(torch.meshgrid(
        torch.linspace(model.xyz_min[0], model.xyz_max[0], int(model.world_size[0]*factor)),
        torch.linspace(model.xyz_min[1], model.xyz_max[1], int(model.world_size[1]*factor)),
        torch.linspace(model.xyz_min[2], model.xyz_max[2], int(model.world_size[2]*factor))
    ), -1)
    torch.set_default_tensor_type('torch.cuda.FloatTensor')

    model.mask_cache(samp_pts.cuda()).sum()

    samp_pts = samp_pts[model.mask_cache(samp_pts.cuda())]
    
    for rp in tqdm(samp_pts.split(8192, 0)):
        temp=render_kwargs['stepsize']
        render_kwargs['stepsize']=(model.xyz_max[0]-model.xyz_min[0])/model.world_size[0]/factor
        ret = model.forward_1p(rp.cuda(), pseudo_grid, **render_kwargs)
        render_kwargs['stepsize']=temp

        
        if (ret['raw_alpha'].size(0) !=0) and (ret['sampled_pseudo_grid'].size(0) !=0):
            (torch.max(torch.abs(ret['feats']),-1)[0].detach()*ret['raw_alpha'].detach()*ret['sampled_pseudo_grid']).sum().backward()  
            

    max_feats_grid = torch.max(torch.abs(model.k0.grid),1,keepdim=True)[0].detach()
    
    model.importance = pseudo_grid.grad.clone()*max_feats_grid
    # model.importance = pseudo_grid.grad.clone()
    
    model.density.grid.grad = None
    model.k0.grid.grad = None
    torch.save(model.importance, imp_path)
    return 



def init_importance_view(model, render_poses, HW, Ks, ndc, render_kwargs, savedir=None, render_factor=0, cfg=None):
    '''Render images for the given viewpoints; run evaluation if gt given.
    '''
    imp_path = os.path.join(savedir, 'importance_view.pth')
