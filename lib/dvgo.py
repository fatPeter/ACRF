...
class DirectVoxGO(torch.nn.Module):
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, target=None, include_thres=None, **render_kwargs):
        '''Volume rendering
    ...
    def forward_imp(self, rays_o, rays_d, viewdirs, pseudo_grid, global_step=None, **render_kwargs):
        ...
        #     print(ray_pts.min(0)[0])
        #     print(ray_pts.max(0)[0])
        debug = False
    ...
    def forward_1p(self, ray_pts, pseudo_grid, global_step=None, **render_kwargs):
        '''Volume rendering
        @rays_o:   [N, 3] the starting point of the N shooting rays.
        @rays_d:   [N, 3] the shooting direction of the N rays.
        @viewdirs: [N, 3] viewing direction to compute positional embedding for MLP.
    ...
    def forward_eb(self, rays_o, rays_d, viewdirs, k0_grid, global_step=None, target=None, include_thres=None, **render_kwargs):
        '''Volume rendering
        @rays_o:   [N, 3] the starting point of the N shooting rays.
        @rays_d:   [N, 3] the shooting direction of the N rays.
        @viewdirs: [N, 3] viewing direction to compute positional embedding for MLP.
        ...
        with utils.Timing('-feat query', debug):
            if self.rgbnet_full_implicit:
                ...
                k0 = self.k0.forward_eb(ray_pts, k0_grid)
            if self.rgbnet is None:
                # no view-depend effect
                rgb = torch.sigmoid(k0)
            else:
    ...
    @torch.no_grad()
    def init_cdf_mask(self, thres_mid=1.0):
        print("start cdf three split")
        importance = self.importance.flatten()   

        if thres_mid==1:
            mask = torch.ones_like(importance).bool()
        elif thres_mid==0:
            mask = torch.zeros_like(importance).bool()
            mask = True # for code robustness issue
        else:
            ...
            mask = importance>split_val_nonprune 
    ...
    @torch.no_grad()
    def ac_reformat(self, non_prune_mask, save_path=None):
        ...
        non_prune_density = density_grid[non_prune_mask,:]
        ...
        new_densiy_grid[non_prune_mask,:] = non_prune_density.dequantize() 
        ...
        if save_path is not None:
            ...
            np.savez_compressed(f'{save_path}/extreme_saving/non_prune_mask.npz',np.packbits(non_prune_mask.reshape(-1).cpu().numpy()))
